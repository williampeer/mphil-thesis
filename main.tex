...
\abstract{
    Computational models have been used to illuminate and unravel neural functioning, with seminal works including models such as the Hodgkin-Huxley model.
    Models are themselves hypotheses of the system we wish to study, and may both explain functional aspects about recorded areas, and be used to generate hypotheses that may be tested in vivo or vitro.
    ...
    Automating this arduous construction of biologically realistic or plausible models could potentially greatly accelerate computational research within neuroscience.
    Therefore, various statistical approaches have been used to try and aid in such modelling, with the current state-of-the-art being based on Bayesian approximation.
    However, this approach does not scale well with growing resolution, i.e. a growing number of neurons.
    Thus, in the era of machine learning, where modeling is shifted towards the data driven using optimisation, it is only natural to ask whether the time may be ripe for us to look at a synthesis where we may leverage a similar approach in computational neuroscience.
    To this end we implement a modular gradient based optimisation framework on top of PyTorch, a modern ML Python library, and investigate to what extent gradient based optimisation (GBO) may be used for spiking neural network (SNN) model inference.
    GBO is of particular interest because the procedure scales well with the network size, due to an "in-place" parameter inference by error gradient signal propagation.
